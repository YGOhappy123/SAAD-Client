...
export default ({ enabledFetchProducts }: { enabledFetchProducts?: boolean }) => {
    ...
    const [products, setProducts] = useState<ITopping[]>([])
    ...
    const buildQuery = (values: {
        ...
        const { isAvailable, searchPriceQuery, searchNameVi, searchNameEn, sort, range } = values
        const query: any = {}
    ...
    const searchProductsQuery = useQuery(['search-products', query, sort], {
        queryFn: () =>
            axios.get<IResponseData<ITopping[]>>(
                `/product/getAllToppings?skip=${itemPerPage * (current - 1)}&limit=${itemPerPage}&filter=${query}&sort=${sort}`
            ),
